...
from smart_contracts.artifacts.kontour_governance.kontour_governance_client import (
    KontourGovernanceClient,
)
...
def deploy() -> None:
    ...
    """
    algorand_client = algokit_utils.get_algod_client(
        algokit_utils.get_default_localnet_config()
    )
    ...
    sp = algokit_utils.ApplicationSpecification.from_artifacts_file(
        "kontour_governance"
    )

    response = client.deploy(
        app_spec=sp,
        on_update=algokit_utils.OnUpdate.append_application,
        on_schema_break=algokit_utils.OnSchemaBreak.append_application,
    )
